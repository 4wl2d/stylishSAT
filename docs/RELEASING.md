...
4. Install the signed APK on a dedicated test device. Test launch, lessons, practice, audio, saved drafts and restart. For native/AI changes, rerun the relevant optional-model suite. Record exact APK hashes and evidence boundaries in release notes; do not attribute old benchmarks to a new APK.
